...
## PELAJARAN PENTING — CI Merah 10 Commit Tanpa Disadari (22 Agu 2026)
...
**Perbaikan:** migrasi baru `20260827605000` (timestamp SEBELUM 20260827610000) menjamin `company_id=1` selalu ada sebelum migrasi `build_tasks` manapun berjalan — idempoten (`ON CONFLICT DO NOTHING`), tidak menyentuh data nyata (diverifikasi `created_at` company_id=1 tidak berubah). **Dibuktikan 2 kali**: (1) reset staging dari nol — 172 migrasi berhasil penuh, 282 detik; (2) CI sungguhan (commit `57637fe`) — kedua job HIJAU.

**Pelajaran untuk sesi berikutnya — WAJIB dipatuhi:** `supabase db push --linked` HANYA membuktikan migrasi valid untuk database yang SUDAH ADA isinya — TIDAK membuktikan migrasi bisa membangun skema dari nol. **Cek status CI (`https://api.github.com/repos/alvhyzid/mrp/actions/runs`) setelah SETIAP push**, jangan menunggu diminta. Kalau menulis migrasi yang menyisipkan/mengubah data (bukan cuma DDL) dengan `company_id` tertentu, JANGAN hardcode literal — pakai pola aman (lookup dinamis + no-op kalau tidak ada) SEPERTI migrasi `build_tasks` lama, atau pastikan dependency data itu (mis. company_id=1) dijamin ada lewat migrasi tersendiri.

**Koreksi terkait:** `tests/attendance_geo_qr_w1.test.ts` sempat dicatat sebagai ABS-05 (gagal 401 konsisten, dikira bug pre-existing) — TERNYATA CI menjalankan file yang SAMA dan LULUS BERSIH. Dugaan kuat: artefak clock/jam sandbox lokal (bukan bug produk), diturunkan urgensinya, BELUM ditutup total (perlu 1 konfirmasi lagi).
