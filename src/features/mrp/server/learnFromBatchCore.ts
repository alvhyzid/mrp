...
// standar produksi. Diekstrak dari learnFromBatch.ts (endpoint HTTP-nya) supaya
// logic yang SAMA bisa dipanggil juga dari completeProductionBatch.ts — batch
// yang ditandai selesai lewat state machine HARUS otomatis diajukan sebagai
// sampel K8, bukan menunggu klik terpisah.
export async function learnFromBatchCore(adminClient: SupabaseClient, companyId: number, productionBatchId: number): Promise<CoreResult> {
