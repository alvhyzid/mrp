...
- **`payroll_period_start_day` (21 Agu 2026, Bagian E)** — tanggal mulai periode gaji pabrik (mis. `26` = periode gaji berjalan tanggal 26 bulan sebelumnya s/d 25 bulan berjalan, BUKAN bulan kalender). **Disimpan sebagai konfigurasi, TAPI BELUM dipakai di mana pun** — `get_monthly_operating_profit()` (Laba Operasional bulanan) MASIH mengelompokkan berdasar BULAN KALENDER (`extract(month from shipments.shipment_date)`). Keputusan apakah Laba Operasional bulanan sebaiknya ikut periode gaji atau tetap bulan kalender SENGAJA belum diambil sepihak — lihat opsi di `HANDOFF.md`, menunggu keputusan pemilik produk.
