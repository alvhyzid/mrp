...
| **Margin/profit per pengiriman** (sistem keuangan inti: pendapatan − biaya bahan − biaya SDM, dihitung saat barang dikirim) | 🟡 Sesi 3A+3B (17 Agu 2026): pengiriman NYATA sudah bisa dibuat & dikelola LEWAT UI oleh Warehouse/PPIC (halaman `/shipments`) — pilih SO bersisa qty, saran lot FEFO otomatis (bisa diganti), alamat tujuan wajib per pengiriman (beda-beda meski SO/customer sama), transisi draft→shipped SEKARANG WAJIB upload foto bukti pengiriman dulu (17 Agu 2026, lihat baris Tanda Tangan Digital) — stok berkurang tepat saat itu, diverifikasi →delivered. Diverifikasi end-to-end lewat browser sungguhan termasuk pengiriman PARSIAL 2x dengan alamat berbeda + percobaan kirim melebihi sisa DITOLAK dengan pesan jelas dari database. BELUM ADA: fungsi kalkulasi margin itu sendiri (belum digarap sama sekali) |
| **Tanda Tangan Digital pada dokumen** (bukan fitur MRPeasy, tapi requirement Anda — persetujuan bertanda tangan sebelum dokumen dikunci) | 🟡 Sesi 1+2 (17 Agu 2026): fondasi GENERIK (upload/ganti tanda tangan, file lama TIDAK PERNAH terhapus; `ConfirmAndSignModal`) SEKARANG dipakai nyata di wizard "Buat Pengiriman" (halaman `/shipments`) — Langkah 1 (form modal Carbon-style, 3 section berjudul: Detail Sales Order/Produk/Detail Pengiriman) → Langkah 2 (preview dokumen Surat Jalan sungguhan + checkbox "Sudah di cek dan tambahkan tanda tangan saya" + Kembali/Buat Pengiriman). Submit = 1 transaksi (shipment+lines+tanda tangan), status TETAP draft, stok TIDAK berkurang. Setelah pengiriman dibuat, "Daftar Pengiriman" pakai pola Carbon *Expandable Data Table* — kolom Status berlabel Draft/Di Proses/Terkirim/Batal (istilah sehari-hari, bukan nama status database), kolom Aksi cuma tombol "Detail" yang membuka panel langsung di bawah baris berisi semua info + tombol "Lihat/Cetak Surat Jalan" (halaman terpisah `/shipments/{id}/surat-jalan`, gambar tanda tangan ASLI + tombol Cetak/Simpan PDF) + tombol "Proses Pengiriman" — WAJIB upload foto bukti pengiriman dulu (validasi role+company+status draft di server), baru setelah itu status berubah `shipped` DAN stok berkurang dalam 1 langkah atomik. Begitu status lewat draft, panel detail JUGA menampilkan stok lot TERKINI dan total qty sudah dikirim se-SO-line (bukan cuma baris pengiriman ini) + foto bukti. Semua diverifikasi lewat browser sungguhan (termasuk upload foto asli, cek status berubah Draft→Di Proses, cek sisa qty SO berkurang tepat, cek stok+total di panel detail). BELUM ADA: QR code Bukti Penerimaan di dokumen cetak (Sesi 3C sisa) dan halaman publik Bukti Penerimaan (Sesi 3) |
