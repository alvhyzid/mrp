...
| **Tanda Tangan Digital pada dokumen** (bukan fitur MRPeasy, tapi requirement Anda — persetujuan bertanda tangan sebelum dokumen dikunci) | ✅ Sesi 1+2+3 (17 Agu 2026): fondasi GENERIK (upload/ganti tanda tangan, file lama TIDAK PERNAH terhapus; `ConfirmAndSignModal`) dipakai nyata di wizard "Buat Pengiriman" (halaman `/shipments`) — Langkah 1 (form modal Carbon-style, 3 section) → Langkah 2 (preview Surat Jalan sungguhan + tanda tangan). "Daftar Pengiriman" pakai pola Carbon *Expandable Data Table* — panel detail per baris + tombol "Lihat/Cetak Surat Jalan" (gambar tanda tangan ASLI + QR code Bukti Penerimaan, BARU) + "Proses Pengiriman" (WAJIB foto bukti pengiriman, ditegakkan di LEVEL DATABASE juga sejak 17 Agu 2026 — bukan cuma API — lewat `enforce_status_transition()`). **Sesi 3 (halaman publik Bukti Penerimaan, `/pod/[token]`) SEKARANG SELESAI** — client scan QR di Surat Jalan fisik → halaman TANPA LOGIN menampilkan ringkasan pengiriman (nol harga/biaya) → upload foto+centang konfirmasi → status otomatis jadi "Terkirim" (delivered), link/QR yang sama tidak bisa dipakai lagi. Diverifikasi lewat 5 skenario negatif wajib (akses tanpa login, token tebakan, token dipakai ulang, cek page source nol harga, isolasi lintas company) — semua lolos. BELUM ADA: fungsi kalkulasi margin/profit itu sendiri |
