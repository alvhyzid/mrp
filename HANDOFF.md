...
**Mekanisme kekeliruannya, dikonfirmasi lewat dokumentasi resmi Supabase** ([`supabase.com/docs/guides/platform/backups`](https://supabase.com/docs/guides/platform/backups)): **PITR dan Daily Backups adalah DUA MEKANISME TERPISAH, bukan satu fitur dengan satu saklar.** "Daily Backups" (snapshot fisik harian) berjalan OTOMATIS di paket Pro/Team/Enterprise **TERLEPAS dari status PITR** — dokumentasi resmi: *"We automatically back up all Pro, Team, and Enterprise Plan projects on a daily basis"* dan *"If you enable PITR, we will no longer take Daily Backups"* (keduanya saling meniadakan, bukan PITR=prasyarat backup). Audit 22 Agu memeriksa **HANYA satu field** (`pitr_enabled`, via API) — field itu MEMANG `false` (benar, PITR memang tidak diaktifkan) — tapi kekeliruannya adalah **mengekstrapolasi "PITR mati" jadi "backup apa pun mati"**, tanpa memeriksa sinyal yang lebih luas (daftar restore point sungguhan di dashboard). **Catatan jujur, belum terjawab tuntas**: masih belum sepenuhnya jelas kenapa titik pemulihan sudah ada sejak 15 Agu — sebelum project ini pindah ke organisasi Pro (23 Agu) — kemungkinan project sudah berstatus paket berbayar sejak sebelum itu di organisasi lama, tapi ini BELUM dikonfirmasi langsung (bukan diklaim pasti).
...
**Akar penyebab** (dibuktikan lewat reproduksi lokal — `supabase db reset --linked` ke project staging `mrp-rebuild-test-2A` yang di-reset kosong lalu direplay 172 migrasi, IZIN EKSPLISIT pemilik produk, project ref & isi data diverifikasi 2x sebelum tindakan): migrasi `build_tasks` LAMA (`20260827340000` dst) SENGAJA memakai pola aman — cari `company_id` lewat `select ... where name = 'PT ITM'`, no-op kalau company belum ada. Migrasi `build_tasks` yang ditulis MULAI `20260827610000` TIDAK memakai pola itu — `company_id` ditulis literal `1`. Di database yang sudah lama berjalan ini tidak pernah kelihatan salah (company_id=1 dibuat manual lewat `scripts/seed-realcase-itm.js`, DI LUAR migrations/, sejak Sesi 2A) — tapi rebuild dari NOL gagal persis: `insert or update on table build_tasks violates foreign key constraint build_tasks_company_id_fkey`.

**Perbaikan KASUS (single-case fix, hari kejadian):** migrasi baru `20260827605000` (timestamp SEBELUM 20260827610000) menjamin `company_id=1` selalu ada sebelum migrasi `build_tasks` manapun berjalan — idempoten (`ON CONFLICT DO NOTHING`), tidak menyentuh data nyata (diverifikasi `created_at` company_id=1 tidak berubah). **Dibuktikan 2 kali**: (1) reset staging dari nol — 172 migrasi berhasil penuh, 282 detik; (2) CI sungguhan (commit `57637fe`) — kedua job HIJAU.

**Perbaikan KELAS (diminta eksplisit pemilik produk — "tutup kelasnya, bukan cuma kasusnya", 22 Agu 2026, II.1-II.5):**

- **II.1 — Sapu SELURUH migrasi.** Ditemukan **10 file** (semua ditulis di sesi yang sama, `20260827610000` s.d. `20260827790000`) memakai `company_id` literal `1` tanpa pengaman, seluruhnya pada `insert into build_tasks`: `20260827610000`, `20260827630000`, `20260827650000`, `20260827660000`, `20260827680000`, `20260827690000`, `20260827700000`, `20260827720000`, `20260827730000`, `20260827790000`. Semuanya ditambal dengan pola aman yang sama (`do $$ ... select company_id into v_company_id from companies where name = 'PT ITM' limit 1; if v_company_id is null then raise notice ...; return; end if; ... end $$;`). Sapuan diperluas ke SELURUH tabel (bukan cuma `build_tasks`) — nol migrasi lain ditemukan bermasalah.
- **II.2 — Pengawas permanen dipasang**: `tests/migration_hardcoded_tenant_id_watchdog.test.ts`. Menyapu `supabase/migrations/*.sql`, mendeteksi `insert into <tabel> (company_id, ...) values (<literal angka>, ...)` pada tabel SELAIN `companies` sendiri (root tenant bootstrap punya pola beda, `ON CONFLICT DO NOTHING`, dikecualikan dengan sengaja). Parser sadar-kutip & sadar-kurung-bersarang (bukan regex baris naif) — dibuktikan lewat 2 putaran perbaikan nyata saat ditulis: percobaan pertama salah tangkap `numeric(14,4)` dari `CREATE TABLE` di bawahnya (batas statement salah), lalu salah tangkap angka DI DALAM teks bebas berkutip seperti `"(5,00 g)"` (parser tuple naif, bukan sadar-kutip) — keduanya diperbaiki sebelum dianggap selesai. **Dibuktikan bisa merah**: migrasi percobaan ditanam dengan `company_id = 1` literal → pengawas gagal, menyebut file+baris+tabel persis → migrasi percobaan dicabut → hijau lagi. 5/5 test lulus, 0 pelanggaran nyata ditemukan di migrasi produksi saat ini.
- **II.4 — Arkeologi (dilaporkan, TIDAK ada yang diperbaiki, sesuai instruksi eksplisit "laporkan dulu, jangan perbaiki").** (a) Rebuild dari nol murni migrasi (tanpa skrip apa pun) SEKARANG menghasilkan **1 company** (PT ITM, `company_id=1`, `industry_type='manufacturing'`, `status='trial'`) — TAPI ini efek SAMPINGAN dari migrasi `20260827605000` (dibuat untuk perbaikan kasus CI hari ini), BUKAN desain onboarding yang disengaja sejak awal; sebelum migrasi itu ada, rebuild dari nol tidak menghasilkan company sama sekali (company_id=1 hanya pernah dibuat lewat `scripts/seed-realcase-itm.js`, di luar migrations/). (b) Tenant KEDUA (dan seterusnya) lahir lewat **PENDAFTARAN ASLI DI APLIKASI** — dikonfirmasi dengan membaca kodenya langsung, bukan menduga: `app/api/register/route.ts` memanggil `registerCompanyAdmin.ts` (`src/features/auth/server/`), yang melakukan `supabase.auth.signUp()` lalu `insert into companies (name, industry_type, status)` biasa (company_id serial otomatis, TIDAK di-hardcode) dan `insert into users (...)` dengan role `company_admin` — jalur ini TIDAK membutuhkan developer atau skrip apa pun, murni alur pengguna lewat form pendaftaran. (c) Karena jawaban (b) adalah "lewat aplikasi, bukan skrip", syarat untuk mencatat task baru ("bila jawabannya butuh skrip") TIDAK terpenuhi — **tidak ada task baru dicatat untuk item ini**, sesuai instruksi ("catat HANYA bila jawabannya butuh skrip").
- **II.5 — ABS-05, dugaan "artefak jam sandbox" DICABUT** (bukan dikonfirmasi) — lihat migrasi `20260827810000`. Jam sandbox dicocokkan ke sumber luar: selisih 1 detik, bukan penjelasan yang masuk akal untuk 401 konsisten. Test dijalankan ulang 3x di sandbox berbeda: lulus 11/11 ketiga kalinya tanpa perubahan kode. Penyebab kegagalan asli TETAP TIDAK DIKETAHUI — urgensi dikembalikan ke Penting, task TETAP terbuka (tidak ditutup hanya karena kebetulan hijau sekarang).

> **PELAJARAN TETAP (kutip persis, jangan diringkas ulang — ini kejadian KETIGA dengan pola sama):**
>
> "`db push` menguji migrasi di atas database yang SUDAH BERISI. CI membangun dari NOL. Keduanya jalur berbeda, dan sesuatu bisa lolos di satu jalur sambil gagal di jalur lain. Ini kejadian ketiga dengan pola sama — sesuatu terlihat benar karena diuji di keadaan yang sudah matang, bukan dari nol. Dua sebelumnya: REVOKE yang terlihat hilang di berkas tapi aman di database, dan snapshot yang aman di batch lama tapi tidak di batch baru. Aturan: setelah menulis migrasi baru, uji rebuild dari nol — jangan hanya `db push`."
...
**BAGIAN 3 (Fondasi PO Supplier) — arkeologi menemukan alur INTI sudah lengkap sejak PMB-01** (trigger `process_goods_receipt_line()`, migrasi 20260812154500): konversi UOM (`uom_conversion_factor`), lot dari harga TRANSAKSI PO (`purchase_order_lines.unit_price`) BUKAN harga acuan supplier (`supplier_item_prices.reference_price`), sisa PO per baris — semua sudah bekerja, dibuktikan ulang (bukan dibangun ulang) di `tests/bagian3_po_supplier_goods_receipt.test.ts`. 1 celah nyata ditambal: `createPurchaseOrder.ts` sekarang menolak supplier yang sudah diarsipkan (pola sama seperti PMB-07c untuk plant nonaktif). 1 keputusan bisnis DITANYAKAN (bukan ditebak) ke pemilik produk: penerimaan barang melebihi qty dipesan — dijawab **IZINKAN + PERINGATAN + CATAT** (bukan gerbang keras, karena barang sudah ada fisik di gudang saat sistem tahu) — diimplementasikan sekaligus: tabel baru `goods_receipt_overage_log` (siapa/kapan/qty dipesan/qty diterima total/qty lebih, kelebihan menyangkut uang untuk Finance), respons API `createGoodsReceipt.ts` menyertakan `warnings` menyebut angka persis. Layar untuk melihat daftar kelebihan BELUM dibangun (menunggu cetakan UX, sesuai batas blok kerja ini).

**BB.2 (putaran 2)**: BELUM dilanjutkan di giliran ini (fokus dialihkan ke Bagian 2/3 sesuai instruksi eksplisit "kerjakan berurutan"). Posisi berhenti TETAP di baris 1863 (lihat entri putaran 1 di bawah) — belum bergerak.


## BAGIAN 5 (PMB-07c) & BAGIAN 6 (H.4/AUD-04) — 22 Agu 2026

**PMB-07c (Gudang/Pabrik Penerima PO Supplier) — SELESAI.** Arkeologi: `production_plant_id` SUDAH wajib+tervalidasi+tertampil sejak sebelumnya. Celah nyata satu-satunya (5.3): plant `is_active=false` masih bisa dipilih & lolos ke server. Ditambal 2 lapis — `createPurchaseOrder.ts` menolak dengan pesan jelas menyebut nama plant; dropdown Lokasi Pabrik di `PurchasingPage.tsx` memfilter plant nonaktif. Bukti: test baru di `tests/pmb07a_identity_snapshot.test.ts` + verifikasi visual `company.b@debug.mrp` (1 plant aktif+1 nonaktif dibuat, dropdown hanya tampilkan yang aktif, fixture dibersihkan total). Dokumen cetak PO Supplier sendiri **belum ada** di aplikasi (beda dari Surat Jalan) — dicatat jujur sebagai di luar cakupan task ini, bukan diklaim selesai.

**AUD-04/H.4 (Kenapa Halaman Pelanggan Lolos Audit 2 Kali) — SELESAI, hipotesis awal terbukti salah alamat.** Dugaan awal ("dropdown dikira jalan masuk") **tidak terbukti** — `git show` ke commit asli tiap versi `docs/audit-lubang-ui.md` menunjukkan baris `customers` SUDAH BENAR sejak audit pertama (Sesi 5) DAN tetap benar di audit ulang berbasis introspection skema (Sesi 7). Akar penyebab sebenarnya, dua hal: (1) **tidak ada jalur mekanis dari temuan-benar di dokumen audit ke `build_tasks`** — task yang akhirnya membangun halaman ini (PMB-03) `origin='pemilik_produk'`, lahir dari permintaan langsung, bukan promosi otomatis dari audit; (2) `docs/checklist-audit-jalan-kaki.md` disusun per **alur-kerja-harian per peran**, tidak ada peran Sales/Pelanggan sama sekali — master data yang jarang dipakai struktural tidak akan pernah muncul di audit bergaya "jalan kaki satu hari kerja". Penjelasan lengkap ditulis di bagian atas `docs/audit-lubang-ui.md`.

**Sinkronisasi manual audit↔tugas dijalankan satu kali**: setiap baris `[MASTER]`/`[X]` di `docs/audit-lubang-ui.md` (Buat=ya, Keluar bermasalah) dicocokkan ke `build_tasks`. **7 temuan sudah benar di dokumen tapi belum pernah jadi task** — dicatat: `DOC-03` (tombol hapus dokumen, backend sudah lengkap), `ABS-03` (approve/revoke perangkat absensi), `ABS-04` (form web pengajuan koreksi absen/cuti, beda dari HR-26 ESS Mobile), `PRD-12` (kolom `work_orders.status` dikonfirmasi 0 baris kode yang pernah meng-update-nya, tapi tetap ditampilkan ke user sebagai label yang menyesatkan — perlu keputusan pemilik produk soal aturan transisi), `PRD-13` (edit field lain batch produksi selain tanggal), `PRD-14` (pin/override manual standar usang), `KPI-03` (isi target & arsip KPI, backend `updateKpiTarget`/`updateKpiVisibility` sudah ada tinggal disambungkan).

**Full suite**: 42 file, 258→259 test (1 test baru PMB-07c). 1 kegagalan transien di 1 run penuh (rate-limit `auth.admin.createUser` di bawah beban — pola sudah pernah terlihat sebelumnya, BUKAN regresi; file yang sama lulus bersih 4/4 saat dijalankan sendiri sebelum dan sesudah).
...
**Keputusan teknis penting**: resolusi `delivery_address_id`→teks alamat dilakukan di **TypeScript** (`createShipmentWithSignature.ts`, sebelum memanggil RPC) lalu `UPDATE shipments.delivery_address_id` terpisah SETELAH RPC sukses — BUKAN menambah parameter ke RPC `create_shipment_with_signature()`. Alasan: fungsi itu SUDAH PERNAH mengalami regresi grant persis akibat penambahan parameter (lihat entri Alur 1) — sengaja tidak diulang.

**Dibuktikan** `tests/pmb07b_delivery_addresses.test.ts` (5 test): pesanan 2.500 dipecah 1.000+1.500 ke 2 alamat tersimpan berbeda → sisa berubah 2500→1500→0 lewat `listSalesOrders`, dua surat jalan dengan alamat masing-masing benar; kirim 1.000 lagi setelah sisa nol → ditolak database; ubah alamat tersimpan SETELAH surat jalan terbit → surat jalan lama tidak berubah; 2 token POD berbeda tidak bisa saling dipakai; alamat yang sudah dipakai pengiriman tidak bisa dihapus permanen (diarsipkan).

**Insiden selama pengerjaan (bukan bug PMB-07b)**: fixture bentrok LAGI — kali ini disebabkan langkah pembersihan manual saya sendiri yang gagal SEBAGIAN tanpa terdeteksi (delete `users` gagal karena FK `document_signatures`, tapi skrip tetap mencetak "berhasil" karena tidak memeriksa error). Baris `users` yang tertinggal dengan email yang sama menyebabkan insert baris baru gagal diam-diam (unique constraint email), sesi berikutnya login dengan `auth_uid` yang tidak cocok baris `users` manapun → 401. **Ini contoh KONKRET ketiga kalinya persis fenomena QA-01** — memperkuat urgensi SUPER URGENT-nya (lihat catatan V.2).

**Test: 41 file/253 test → 42 file/258 test** (+1 file, +5 test). Docs (`rancangan-skema-database-mrp.md`, `daftar-database-sederhana.md`) diperbarui. Route API baru: `app/api/customer-delivery-addresses/**`.
...
**INF-07 (investigasi "data berubah tanpa pelaku") — DIBUKA KEMBALI 23 Agu 2026, kesimpulan sebagian DIKOREKSI.** Kesimpulan asli 22 Agu ("BUKAN data nyata yang berubah, dibuktikan lewat snapshot idle 35 menit") **TETAP VALID sebagai bukti** — tapi HANYA membuktikan tidak ada tulisan SAAT TIDAK ADA YANG BEKERJA. Ia tidak pernah diuji terhadap keadaan saat test suite berjalan BERSAMAAN. Ditemukan 23 Agu 2026 (persiapan transfer Supabase, S.2): jumlah baris tabel kunci project data nyata BERUBAH antar dua snapshot berselang detik (`users` 8→9→10, dst), dengan baris ber-pola nama test ("attendancew1test"/"stepprogresstest") tercipta detik itu juga — padahal tidak ada proses test di komputer kerja. **Root cause dikonfirmasi (bukan dugaan)**: test lain sedang berjalan BERSAMAAN terhadap project SUNGGUHAN, menciptakan company barunya sendiri (BUKAN company_id=1 — Invariant 9 tetap terbukti aman) yang bersih sendiri sempurna dalam <2 menit begitu test itu selesai normal. **Gap sebenarnya**: pengaman yang ada (Invariant 9) melindungi BARIS company_id=1, tapi tidak ada satu pun yang mencegah test suite dijalankan terhadap PROJECT yang sama dengan rumah data nyata sejak awal — karena `.env.local` yang dipakai `npm run dev` (lihat INF-11) adalah `.env.local` yang SAMA dibaca `npx vitest run`. **DITAMBAL hari ini**: `tests/setup/guardAgainstRealProject.ts` (INF-12, SELESAI) — menolak keras test suite lokal berjalan terhadap project yang terdaftar sebagai rumah data nyata kecuali `ALLOW_TESTS_AGAINST_REAL_PROJECT=true` diset eksplisit; ditemukan sekalian bahwa CI ("Typecheck & Test Suite") JUGA memakai project sungguhan yang sama (`secrets.NEXT_PUBLIC_SUPABASE_URL`) — flag yang sama ditambahkan EKSPLISIT di `ci.yml` (bukan pengecualian tersembunyi berbasis `CI=true`) supaya risikonya terlihat di kode. Dibuktikan bisa merah lalu hijau (tanpa flag → gagal keras menyebut nama project; dengan flag → lulus normal). INF-06 diperbarui: 5 company tersisa (bukan 4), `AiProjectDashboardTestCorp` (22 Agu) baru ditemukan lewat sinkronisasi ini. Verifikasi Production Vercel diulang (bundle JS live, tanpa kredensial): masih menunjuk project Supabase KOSONG, bukan data nyata — aman (dicatat lebih lanjut sebagai `INF-11`, SUPER URGENT — situs deploy tidak tersambung ke data nyata sama sekali, satu-satunya jalur ke data nyata hari ini adalah localhost). Sweep ulang kelas bug "terlihat baca, sebenarnya tulis" di kode pasca-Sesi 0C (Alur 1, Daftar Tugas Pembangunan, snapshot surat jalan): **tidak ditemukan instans baru** — 2 write-dalam-`get*` yang ditemukan (`upsert_margin_threshold_alert` di `getMarginWatch.ts`, `kpi_snapshots` upsert di `listKpiCards.ts`) sudah ada sebelumnya by design, idempoten, tidak mengunci apa pun ireversibel — beda kelas dari bug asli. **Tidak ada mekanisme audit trail generik untuk `employees`/`companies`** (dikonfirmasi tidak ada trigger/tabel histori) — ini alasan struktural kenapa ketiga anomali lama tidak pernah terpecahkan tuntas; sudah dinaikkan urgensinya jadi Mendesak di task `AUD-07`.
