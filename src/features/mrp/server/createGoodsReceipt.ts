...
export async function createGoodsReceipt(request: NextRequest): Promise<ApiResult> {
  try {
    ...
    for (const line of receiptLines) {
      const { error: lineError } = await adminClient.from('goods_receipt_lines').insert([
        { goods_receipt_id: gr.goods_receipt_id, purchase_order_line_id: line.purchase_order_line_id, item_id: line.item_id, qty_received: line.qty_received }
      ]);
    ...
    return { status: 201, body: { goods_receipt_id: gr.goods_receipt_id, purchase_order_status: newStatus, lines: createdLines } };
  } catch (error) {
