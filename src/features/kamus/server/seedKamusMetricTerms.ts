...
const METRIC_TERMS: { term_key: string; ai_draft: string; domain?: string; suggested_role?: string }[] = [
  ...
  },
  {
    ...
    suggested_role: 'finance',
    ai_draft:
      'KPI (permintaan pemilik produk 25 Agu 2026): Margin Kontribusi (Rupiah) ÷ Total nilai jual × 100. Data SAMA dengan metric.margin_kontribusi, cuma dinyatakan persentase. RIWAYAT TARGET (dicabut 26 Agu 2026): sempat dipasang target 35% dari kebijakan GPM tim finance untuk konteks simulasi PO lama (Gummy Zala/Drinkme) -- pemilik produk memutuskan angka itu KHUSUS konteks lama, BUKAN kebijakan yang berlaku untuk studi kasus baru, jadi target_value dikosongkan lagi (baseline dulu, target kemudian, sama seperti KPI lain). CATATAN METODOLOGI yang TETAP RELEVAN kalau target GPM dipasang lagi kelak: GPM (Gross Profit Margin) dihitung setelah SELURUH harga pokok termasuk overhead pabrik, sedangkan Margin Kontribusi belum dikurangi overhead (aturan K2 -- overhead hanya masuk di Laba Operasional bulanan) -- Margin Kontribusi % akan SELALU LEBIH TINGGI dari GPM sesungguhnya, bukan pengukur GPM yang presisi.'
  },
  {
