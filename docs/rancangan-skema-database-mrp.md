...
- **`default_min_stock_percent` (25 Agu 2026, MST-27 / KK.3)** — persen ambang stok minimum yang berlaku untuk SELURUH item perusahaan. Ambang efektif per item dihitung TIGA LAPIS di `src/features/mrp/stockThreshold.ts` (`tentukanAmbang`), urutannya mengikat: (1) `items.min_stock_percent` bila terisi > 0, (2) kunci ini bila terisi > 0, (3) `items.min_stock_level` (angka mutlak, warisan). Perusahaan yang belum mengisinya berperilaku PERSIS seperti sebelum kunci ini ada — nol peringatan baru muncul diam-diam. **Alasan lapis perusahaan ditambahkan**: kolom persen per item sudah lama tersedia dan tidak pernah sekali pun diisi; isian yang harus diisi ratusan kali tidak akan diisi. **`memengaruhiHistoris: true`** di katalog setelan karena ia mengubah ARTI peringatan, bukan tampilannya.
...
> **`users.avatar_url` — nama berkas UNIK sejak 25 Agu 2026 (FND-04).** Foto profil disimpan di bucket `user-avatars` dengan nama `<auth_uid>/avatar-<waktu>-<acak>.<ext>` dan `upsert: false`. Sebelumnya namanya **tetap** (`avatar.<ext>`, `upsert: true`), sehingga mengganti foto dengan format yang sama **menimpa yang lama tanpa jejak** — dan itulah sebab foto pemilik produk hilang permanen. **Foto lama sengaja TIDAK dihapus** saat unggahan baru; pembersihannya menyusul lewat INF-23.
...
- `address`, `npwp`, `pic_name`, `pic_phone`, `pic_email`, `payment_terms` (Alur 1, 21 Agu 2026 — koreksi pemilik produk, SEMUA nullable kecuali `name`: field wajib yang tidak dipakai adalah gesekan harian)
- `archived_at`, `archived_by` (→ `user_id`) (Alur 1) — pola sama Routing (Sesi 7): NULL = aktif & boleh dipilih PO Supplier baru. "Hapus" permanen hanya kalau TIDAK direferensikan `purchase_orders` maupun `supplier_item_prices` sama sekali; kalau sudah dipakai, hanya "Arsipkan", ditegakkan di server (`deleteOrArchiveSupplier.ts`), bukan pilihan pengguna.

### `supplier_item_prices` (Alur 1, 21 Agu 2026)
Daftar "bahan yang dipasok" — hubungan BANYAK-ke-BANYAK `suppliers` × `items`. SENGAJA bukan kolom di `items` (satu bahan boleh dipasok banyak supplier dengan harga/lead time beda) dan BUKAN cara membuat item baru (item bahan baku lahir HANYA dari tindakan sadar "buat bahan baru" di layar Item — prinsip yang sama dengan larangan "menyimpan diam-diam" Sesi 0C). SATU baris per (`supplier_id`, `item_id`) — diperbarui di tempat saat harga berubah, bukan riwayat bertingkat.
- `supplier_item_price_id`, `company_id`, `supplier_id` (→ `suppliers`), `item_id` (→ `items`, WAJIB dipilih dari master, tidak boleh teks bebas)
- `supplier_item_code`, `supplier_item_name` (nullable — sebutan/kode barang ini menurut supplier, sering beda dari nama internal kita)
- `reference_price` (nullable, numeric) — **HARGA ACUAN, BUKAN HPP.** Hanya untuk perencanaan (perkiraan margin sebelum PO terbit, banding-banding supplier). Di layar mana pun harga ini muncul, WAJIB berlabel "harga acuan supplier — belum ada pembelian nyata". HPP tetap dari `lots.unit_cost` hasil penerimaan barang (K5) — harga acuan TIDAK PERNAH masuk perhitungan biaya batch.
- `price_valid_from` (nullable, date), `min_order_qty`, `min_order_uom`, `lead_time_days_override` (nullable — menimpa `suppliers.lead_time_days` khusus bahan ini), `notes`
- **Dua pintu masuk, satu data**: diisi dari layar Supplier ("+ Tambah Bahan", `PurchasingPage.tsx`) ATAU dari layar Item ("Pemasok", `ItemsPage.tsx`) — keduanya memanggil `upsertSupplierItemPrice.ts` yang sama.
- **Garis tegas 3.5**: `computeStandardCostPerUnit.ts` BOLEH mengestimasi biaya standar item yang `standard_cost`-nya NULL dari `reference_price` termurah (ditandai `estimatedFromReferencePriceItemCodes`) untuk PREVIEW Margin Watch — TAPI `lockMarginBaseline.ts` MENOLAK mengunci baseline selama daftar itu tidak kosong, pesan sebut persis bahan mana. Keputusan pemilik produk: baseline hanya berdiri di atas harga yang benar-benar dibayar.
...
- `contact_info`
- `billing_address`, `shipping_address` (nullable, boleh beda), `npwp`, `pic_name`, `pic_phone`, `pic_email`, `payment_terms` (Alur 1, 21 Agu 2026 — sama seperti `suppliers`, semua nullable kecuali `name`)
- `archived_at`, `archived_by` (→ `user_id`) (Alur 1) — pola sama Routing/Supplier: "Hapus" hanya kalau TIDAK direferensikan `customer_purchase_orders` sama sekali, kalau sudah dipakai hanya "Arsipkan", ditegakkan server (`deleteOrArchiveCustomer.ts`).

### `customer_delivery_addresses` (PMB-07b, 22 Agu 2026 — LAPISAN DATA & SERVER SAJA, belum ada halaman)
Alamat tujuan kirim SEBAGAI DAFTAR per customer (mis. beberapa distributor) — sengaja TERPISAH dari `customers.shipping_address` (satu alamat tetap lama, tidak dihapus). Alamat tujuan **milik pengiriman, bukan milik pelanggan**: dipilih manual saat shipment dibuat, TIDAK diwarisi otomatis dari customer, TIDAK dianggap tetap.
- `customer_delivery_address_id`, `company_id`, `customer_id`
- `label` (nama panggilan, mis. "Distributor Surabaya"), `address`, `pic_name` (nullable), `pic_phone` (nullable)
- `archived_at`, `archived_by` (→ `user_id`) — pola sama Supplier/Customer: "Hapus" hanya kalau TIDAK dipakai `shipments.delivery_address_id` mana pun, kalau sudah dipakai hanya "Arsipkan" (`customerDeliveryAddresses.ts`).

> **Arkeologi dilakukan sebelum membangun ini (dicatat supaya tidak diulang):** blokir kelebihan-kirim (`enforce_shipment_line_qty_limit()`, migrasi `20260817150000`) TERBUKTI SUDAH kumulatif per `sales_order_line_id` lintas SELURUH shipment (bukan per shipment) — TIDAK perlu diperbaiki. `shipments.delivery_address` (teks) SUDAH bebas-diisi per shipment sejak awal (migrasi `20260817140000`) — SUDAH beku, TIDAK diwarisi otomatis. `pod_token` SUDAH `unique` sejak awal (migrasi `20260817180000`) — mustahil 2 shipment berbagi token. `sales_order_lines.qty_shipped` SUDAH kolom hidup yang diinkremen trigger saat shipment berstatus `shipped` — sisa kirim (`qty_ordered - qty_shipped`) SUDAH dihitung `listSalesOrders.ts` (`qty_remaining_to_ship`). Satu-satunya yang GENUINELY baru: tabel daftar alamat tersimpan ini + kolom jejak di bawah.
...
- `estimated_from_reference_price_item_codes` (Alur 1, 21 Agu 2026, TIDAK disimpan di baseline TERKUNCI — computeStandardCostPerUnit.ts SELALU mengembalikan [] untuk snapshot yang sudah terkunci, lihat lockMarginBaseline.ts) — item yang `standard_cost`-nya NULL tapi punya `supplier_item_prices.reference_price`, dipakai sebagai ESTIMASI TERAKHIR sebelum menyerah ke `missing_cost_item_codes`. Boleh muncul di PREVIEW (belum terkunci), TIDAK BOLEH ADA di baseline terkunci — `lockMarginBaseline.ts` menolak dengan pesan sebut persis bahan mana kalau daftar ini tidak kosong saat mencoba mengunci.
...
- `customer_name_snapshot`, `customer_billing_address_snapshot`, `customer_npwp_snapshot` (nullable, Alur 1, 21 Agu 2026, 3.1b) — **identitas client DIBEKUKAN PERSIS saat shipment ini dibuat** (`create_shipment_with_signature()` RPC, migrasi `20260827300000`), pola sama dengan snapshot routing/BOM Sesi 6A. Sebelum ini, `getShipmentDetail.ts` melakukan JOIN LIVE ke `customers.name` setiap surat jalan dibuka/dicetak — DIBUKTIKAN: mengubah nama/alamat client membuat surat jalan yang SUDAH dicetak & ditandatangani bulan lalu ikut menampilkan versi baru begitu dimuat ulang. NULL untuk shipment lama dari sebelum kolom ini ada — `getShipmentDetail.ts` fallback ke join hidup HANYA untuk kasus itu.
- `delivery_address_id` (nullable, → `customer_delivery_addresses`, PMB-07b 22 Agu 2026) — jejak REFERENSI alamat tersimpan mana yang dipilih (null kalau alamat diketik langsung sekali-pakai). BUKAN sumber teks alamat yang tampil — `delivery_address` (kolom teks di atas) tetap satu-satunya nilai BEKU yang dipakai, diisi lewat `createShipmentWithSignature.ts` (resolusi di TypeScript, BUKAN parameter RPC — RPC ini pernah mengalami regresi grant akibat penambahan parameter, sengaja tidak diulang).
...
> **`low_stock` BUKAN LAGI "stok rendah" saja (GDG-10 / KK.1, 25 Agu 2026).** Sejak keputusan pemilik produk, satu bahan menghasilkan **SATU** peringatan yang menyebut **seluruh** sebab yang menyala: sisa stok di bawah ambang, DAN/ATAU kurang untuk Work Order yang sedang berjalan. Konsekuensinya `low_stock` bisa menyala untuk bahan yang stoknya justru masih **di atas** ambang — dan dalam keadaan itu kalimatnya diawali "PERIKSA DULU", bukan "perlu dipesan". Dibangun di `src/features/mrp/server/refreshLowStockAlerts.ts`; kalimatnya disusun di `src/features/mrp/alasanPeringatanBahan.ts` (murni perhitungan, tanpa database). Label di layar ikut berubah dari "Stok Rendah" jadi **"Perlu dipesan"**, karena label lama menyesatkan di kasus itu.
>
> **`material_shortage` per Work Order DICABUT (25 Agu 2026, keputusan pemilik produk).** `recompute_work_order_material_readiness()` tidak lagi membuat peringatan; ia hanya **menutup** peringatan lama yang masih terbuka. Peringatan gabungan per bahan kini **menyebut sendiri** perintah produksi mana yang tertahan (nomor batch bila sudah ada, kalau belum kode produk + kuantitas rencana). Jenis `material_shortage` **tetap ada di CHECK constraint** supaya baris lama tetap punya arti saat dilihat — bukan karena masih dipakai.
>
> **PENGGANTI YANG WAJIB ADA LEBIH DULU**: baris peringatan itu dulu satu-satunya hal yang membuat sebuah Work Order tampil **"Terhambat"** (`work_orders_readiness` menyatakan blocked bila ada peringatan terbuka yang menunjuk `work_order_id`). Karena itu view-nya **diubah lebih dulu** supaya menghitung kekurangan bahan **langsung dari `bom_lines` dan `lots`**, dengan kolom baru `kekurangan_bahan`, dan `tests/kesiapan_wo_dari_data.test.ts` membuktikan kesiapannya tetap terbaca `blocked` **dengan nol baris peringatan di tabel**. Baru setelah itu peringatannya dicabut.
>
> **Beda stok yang disengaja**: view kesiapan memakai stok **PER PLANT** (satu Work Order hanya bisa memakai bahan di pabriknya sendiri); peringatan bahan memakai stok **LINTAS PLANT** (yang dinilai bahannya, dan bahan di gudang sebelah tetap milik perusahaan). Dua pertanyaan berbeda, bukan ketidakkonsistenan.
