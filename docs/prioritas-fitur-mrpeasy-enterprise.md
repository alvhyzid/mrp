...
| Tampilan telusur 1 nomor PO (BOM, status, jadwal, progres, sisa kirim) | 🟡 `SalesOrdersPage` sudah punya list + detail (SO, WO terkait, progres). Fondasi data "sisa kirim" sudah ada sejak 17 Agu 2026 (`sales_order_lines.qty_shipped` ter-update otomatis, diverifikasi trigger nyata) — TAPI belum ada UI yang menampilkannya di halaman ini (menyusul Sesi 3B) |
...
| **Margin/profit per pengiriman** (sistem keuangan inti: pendapatan − biaya bahan − biaya SDM, dihitung saat barang dikirim) | 🟡 Sesi 3A (17 Agu 2026): fondasi pengiriman NYATA sudah jalan — status `draft→shipped→delivered` (state machine di database), stok berkurang TEPAT saat "shipped" (bukan saat baris dibuat), traceability lot WAJIB, `stock_movements` tercatat, semua diverifikasi lewat skenario nyata (termasuk 3 skenario gagal: lot kosong ditolak, lompat status ditolak, stok fisik tidak cukup ditolak). BELUM ADA: UI untuk staf benar-benar membuat pengiriman (Sesi 3B) dan fungsi kalkulasi margin itu sendiri (belum digarap sama sekali) |
