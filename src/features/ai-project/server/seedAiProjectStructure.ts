...
export async function seedAiProjectStructure(adminClient: SupabaseClient, companyId: number): Promise<{ phasesInserted: number; tasksInserted: number; totalTasks: number }> {
  ...
  const tasks: TaskSeed[] = [
    ...
    },
    {
      ...
      description: '[Direklasifikasi dari AUTO_QUERY ke CHECKLIST 21 Agu 2026. Tipe ProvenanceEnvelope + komponen ProvenanceInfoButton generik SUDAH dibangun (src/lib/provenance.ts, src/components/ui/provenance-info-button.tsx) -- checklist di bawah = target uji "20 angka lintas modul bisa dijelaskan" dari dokumen sumber. SELESAI 20/20 (25 Agu 2026) -- item ke-19 (yield batch produksi) sebelumnya tidak punya panel UI sama sekali (bukan cuma kurang tombol provenance), jadi ditambah kolom total_output_qty di listWorkOrders + tampilan "Yield Aktual vs Rencana" baru di ProductionDashboardPage.]',
      weight_percent: 15,
      owner_type: 'CLAUDE_CODE',
      suggested_role: null,
      progress_source: 'CHECKLIST',
      ...
      checklist: [
        ...
        { label: 'Margin Watch: selisih harga bahan/kemasan (Lapis 2)', done: true },
        { label: 'Margin Watch: selisih pemakaian bahan (Lapis 2)', done: true },
        { label: 'Margin Watch: selisih reject (Lapis 2)', done: true },
        ...
        { label: 'Kelayakan jadwal: tanggal selesai proyeksi', done: true },
        { label: 'BOM: biaya standar per komponen', done: true },
        { label: 'Item: standard_cost', done: true },
        { label: 'Laba Operasional: margin kontribusi bulanan', done: true },
        { label: 'Laba Operasional: overhead SDM bulanan', done: true },
        { label: 'Work Order: kebutuhan bahan per batch (buffer_percentage)', done: true },
        { label: 'Batch produksi: yield aktual vs rencana', done: true },
        { label: 'Employee: biaya pemberi kerja per bulan (BPJS uplift)', done: true },
