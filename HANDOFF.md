...
### 9. Pemeriksaan a-d (laporan produksi harian nyata, 13 Agu 2026 — CONTOH FORMAT, bukan data SAS005 sungguhan, lihat poin 10) — LAPORAN SAJA, TIDAK ADA YANG DIBANGUN
- **(a) Progres tahap tanggal berbeda dari hari pencatatan — BLOCKER NYATA, dikonfirmasi.** `recordWorkOrderStepProgress.ts` men-hardcode `started_at`/`completed_at` ke `new Date().toISOString()` (waktu server SEKARANG) — TIDAK ADA field di request yang bisa mengisi tanggal actual berbeda. Mencatat hari ini progres Filling untuk batch yang di-mixing 2 hari lalu akan tercatat seolah semuanya terjadi HARI INI. Ini genuinely blocker untuk pola pabrik (batch mengalir lintas hari) — belum diperbaiki (di luar cakupan diminta sesi ini, "jangan bangun dulu").
- **(b) Reject per tahap — GAP, dikonfirmasi.** `work_order_step_progress` cuma punya `qty_input`/`qty_recorded` (susut generik = input−output), TIDAK ADA kolom terpisah untuk "reject" (barang cacat, dibuang) vs susut proses biasa (evaporasi dsb) apalagi reject KEMASAN terpisah (mis. "Total Reject box: 8 box" dari laporan contoh). Sistem TIDAK BISA mereproduksi struktur laporan pabrik yang mencatat reject eksplisit per tahap.
