...
> **Legenda status (per audit 15 Agu 2026, diverifikasi lewat kode + query database langsung, bukan asumsi dari skema — diperbarui 16 Agu 2026 setelah Purchasing/Goods Receipts/Lot Genealogy dibangun):** ✅ = sudah dibangun DAN sudah bisa dipakai lewat UI/alur nyata. 🟡 = sebagian — ada kode/skema/backend nyata, tapi ada bagian penting yang belum bisa dipakai (jelaskan di kolom status). ❌ = belum dibangun sama sekali, baru rancangan di skema/dokumen.
...
| Tampilan telusur 1 nomor PO (BOM, status, jadwal, progres, sisa kirim) | 🟡 `SalesOrdersPage` sudah punya list + detail (SO, WO terkait, progres) — TAPI kolom "sisa kirim" tidak bisa ditampilkan karena `shipments` belum dibangun sama sekali |
...
| **Margin/profit per pengiriman** (sistem keuangan inti: pendapatan − biaya bahan − biaya SDM, dihitung saat barang dikirim) | ❌ Belum ada. `shipments` (0 baris) belum dibangun, dan migrasi tabel-tabelnya TIDAK berisi fungsi kalkulasi margin apa pun — baru rancangan di dokumen |
