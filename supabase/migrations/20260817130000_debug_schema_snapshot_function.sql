-- Migration: fungsi diagnostik untuk mengambil snapshot skema LENGKAP (tabel,
-- kolom, constraint, index, trigger, RLS policy, view, function, sequence) sebagai
-- teks deterministik yang bisa di-diff — dipakai Sesi 2A (uji rebuild-from-
-- migrations, docs/rencana-kerja-playbook-ams.md) sebagai pengganti sementara
-- `pg_dump --schema-only` karena environment kerja saat ini tidak punya Docker/
-- pg_dump terpasang. HANYA untuk service_role, sama seperti debug_list_policies()
-- (migrasi 20260812160000) — bukan untuk dipanggil dari aplikasi.
--
-- CATATAN PENTING: ini solusi SEMENTARA untuk verifikasi manual. Saat Sesi 2C
-- (setup CI GitHub Actions) dikerjakan, uji rebuild-migrasi yang jadi bagian
-- PERMANEN di CI WAJIB pakai pg_dump sesungguhnya (GitHub Actions runner
-- biasanya punya akses Postgres/Docker) — bukan fungsi ini.
--
-- Cakupan (biar sama ketat dengan yang dijamin pg_dump --schema-only):
--   'table'      nama tabel + RLS enabled/disabled
--   'column'     nama kolom + tipe data + nullable + default, per tabel
--   'constraint' SEMUA jenis (primary key/foreign key/unique/check) + definisi
--                persis lewat pg_get_constraintdef() (termasuk ekspresi CHECK
--                apa adanya, bukan cuma "ada check constraint")
--   'index'      definisi lengkap lewat pg_get_indexdef()
--   'trigger'    timing+event+fungsi yang dipanggil lewat pg_get_triggerdef()
--   'policy'     per role, per command (cmd), ekspresi USING dan WITH CHECK lengkap
--   'view'       definisi lengkap lewat pg_get_viewdef()
--   'function'   signature + BODY lengkap lewat pg_get_functiondef()
--   'sequence'   nama sequence + kolom yang memilikinya (owned_by)

drop function if exists public.debug_schema_snapshot();
create function public.debug_schema_snapshot()
