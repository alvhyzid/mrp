...
export async function computeStandardLaborCostPerUnit(adminClient: SupabaseClient, companyId: number, topItemId: number): Promise<LaborCostResult> {
  ...
  for (const [itemId, ratio] of producedRatioPerTopUnit) {
    ...
    notes.push(`${itemLabel}: kru harian ${formatCurrency(dailyCrewCost, { maxDecimals: 0 })} ÷ ${batchesPerDay} batch/hari ÷ ${unitPerBatch} unit/batch = ${formatCurrency(costPerUnitThisLevel)}/unit level ini × rasio ${formatNumberId(ratio, 6)} ke unit teratas.`);
  }
