...
export async function listItems(request: NextRequest): Promise<ApiResult> {
  try {
    ...
    const { data, error } = await adminClient
      .from('items')
      .select(
        'item_id, item_code, name, type, base_uom, purchase_uom, uom_conversion_factor, shelf_life_days, min_stock_level, min_stock_percent, reorder_point, reorder_qty, is_active, standard_cost, bpom_registration_number, halal_certificate_number'
      )
